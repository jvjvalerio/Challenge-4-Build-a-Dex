...
function toChecksumAddress(address) {
    address = encoding_1.removeHexPrefix(address.toLowerCase());
    const hash = encoding_1.removeHexPrefix(js_sha3_1.keccak_256(encoding_2.convertUtf8ToBuffer(address)));
    let checksum = "";
    ...
    return encoding_1.addHexPrefix(checksum);
}
...
function parsePersonalSign(params) {
    if (!validators_1.isEmptyArray(params) && !validators_1.isHexString(params[0])) {
        params[0] = encoding_2.convertUtf8ToHex(params[0]);
    }
    return params;
}
    ...
    function parseHexValues(value) {
        ...
        if (typeof value === "number" || (typeof value === "string" && !validators_1.isEmptyString(value))) {
            if (!validators_1.isHexString(value)) {
                result = encoding_2.convertNumberToHex(value);
            }
